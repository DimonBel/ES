...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		% Program mode
		\node[state, fill=blue!20] (prog) at (9,4.5) {PROG\\Mode};
		
		% Digit accumulation
		\node[decision] (full) at (14,0) {4 digits?};
		\node[decision] (valid) at (14,-5.3) {Code\\Valid?};
		
		% End states
		\node[state, fill=green!30] (granted) at (9,-8.8) {ACCESS\\GRANTED\\Green LED 5s};
		\node[state, fill=red!30] (denied) at (19,-8.8) {ACCESS\\DENIED\\Red LED blink 5×};
		...
		% Programming mode
		\draw[arrow] (input.north) -- node[right, font=\footnotesize] {Hold 'D'} (prog.south);
		\draw[arrow] (prog.west) to[out=180, in=90] node[left, font=\footnotesize, pos=0.45] {Exit} (idle.north);
		
		% Main authentication flow
		\draw[arrow] (input) -- node[above, font=\footnotesize] {digit} (full);
		\draw[arrow] (full) -- node[right, font=\footnotesize] {Yes} (valid);
		\draw[arrow] (full.west) -- node[above, font=\footnotesize] {No} ++(-2.5,0) |- (idle.south);
		
		% Authentication results
		\draw[arrow] (valid.south west) -- node[left, font=\footnotesize, pos=0.4] {Yes} (granted.north east);
		\draw[arrow] (valid.south east) -- node[right, font=\footnotesize, pos=0.4] {No} (denied.north west);
		
		% Return to idle
		\draw[arrow] (granted.south) -- ++(0,-1.5) -| node[below, font=\footnotesize, pos=0.68] {3s delay} (idle.south);
		\draw[arrow] (denied.south) -- ++(0,-3.2) -| node[below, font=\footnotesize, pos=0.9] {3s delay} (idle.south);
		
		% Clear/Reset functionality
		\draw[arrow] (input.south) -- node[right, font=\footnotesize] {'*' Reset} ++(0,-2.8) -| (idle.south);
	\end{tikzpicture}
	}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\draw[arrow] (lcd.west) to[out=180, in=180] node[left, font=\footnotesize, pos=0.5] {display} (user.west);
		\draw[arrow] (led.west) to[out=120, in=60] node[above, font=\footnotesize, pos=0.5] {visual feedback} (user.north);
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\node[process, fill=orange!20] (clear) at (6.8,-2.5) {Clear Buffer\\Reset '*'};
		\node[process, fill=blue!20] (prog) at (6.8,0.2) {Programming\\Mode 'D'};
		...
		\draw[arrow] (reset_success.west) -- ++(-2.2,0) |- (start.west);
		\draw[arrow] (reset_fail.east) -- ++(2.2,0) |- (start.east);
		
		% Special command flows
		\draw[arrow] (check.east) -- node[above, font=\footnotesize] {'*'} (clear.west);
		\draw[arrow] (check.north east) -- node[above, font=\footnotesize, pos=0.35] {'D'} (prog.west);
		\draw[arrow] (clear.east) -- ++(1.6,0) |- (start.east);
		\draw[arrow] (prog.east) -- ++(1.6,0) |- (start.east);
		
		% Partial input handling
		\draw[arrow] (full.east) -- node[above, font=\footnotesize] {No} ++(3.5,0) |- (start.east);
	\end{tikzpicture}
	}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\node[process] (reset) at (22,-7) {RESET\\Welcome};
		
		% Arrows
		\draw[arrow] (init) -- (welcome);
		\draw[arrow] (welcome) -- (key);
		\draw[arrow] (key) -- node[left, font=\scriptsize] {Yes} (d_key);
		\draw[arrow] (key.north) to[out=60, in=120, looseness=7] node[above, font=\scriptsize] {No} (key.north);
		...
		\draw[arrow] (check.north) -- node[right, font=\scriptsize] {No} ++(0,1.6) -| (input.south);
		\draw[arrow] (valid) -- node[above left, font=\scriptsize] {Yes} (ok);
		\draw[arrow] (valid) -- node[below left, font=\scriptsize] {No} (err);
		\draw[arrow] (ok) -- (reset);
		\draw[arrow] (err) -- (reset);
		\draw[arrow] (reset.north) to[out=150, in=20] (welcome.east);
	\end{tikzpicture}
	}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{lab1.2-img/corect-code.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{lab1.2-img/dev-mode.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8.5\textwidth]{lab1.2-img/Hardware-setup.jpg}
...
\begin{itemize}
	\item \textbf{Parallel (4/8-bit):} Fast, but uses many GPIO pins (6--11) and more wiring.
	\item \textbf{SPI:} Very fast and robust, but still needs more pins and is less common for small character LCDs.
	\item \textbf{I2C:} Uses only 2 pins (SDA/SCL), wiring is simple, and speed is enough for text updates.
	\item \textbf{Best choice here:} I2C, because access-control systems need free pins for keypad/LED/sensors and do not require high LCD bandwidth.
\end{itemize}
...
\begin{itemize}
	\item 16 keys arranged in 4 rows and 4 columns
	\item 4 row lines configured as OUTPUT
	\item 4 column lines configured as INPUT\_PULLUP
\end{itemize}

\textbf{Scanning Algorithm:}

\begin{enumerate}
	\item Set all rows HIGH.
	\item Drive one row LOW and read all columns.
	\item If one column is LOW, map (row, column) to a key.
	\item Debounce (about 20ms), wait release, then return the key.
\end{enumerate}
...
\textbf{Debouncing:}
\begin{itemize}
	\item Press debounce: ~20ms
	\item Release debounce: ~50ms
\end{itemize}

\textbf{Advantages:}
\begin{itemize}
	\item Saves pins (8 lines for 16 keys)
	\item Easy to scale to bigger key matrices
\end{itemize}
...
\begin{itemize}
	\item Implement \texttt{lcd\_putchar(char, FILE*)}: send each output char to LCD and handle newline/wrap.
	\item Implement \texttt{keypad\_getchar(FILE*)}: read one debounced key and map it to ASCII.
	\item Bind both handlers to \texttt{stdout} and \texttt{stdin} using AVR stream setup.
\end{itemize}
...
\textbf{Challenges:}
\begin{itemize}
	\item Blocking input flow
	\item Small LCD space (16×2)
	\item Extra memory cost for STDIO
\end{itemize}
...
\begin{itemize}
	\item \texttt{putchar()} is the low-level output hook used by \texttt{printf()}.
	\item \texttt{getchar()} is the low-level input hook used by \texttt{scanf()} and similar APIs.
	\item In redirection, custom handlers send output to LCD and read input from keypad.
	\item Compared to UART: serial is simpler (buffered stream), while LCD/keypad needs cursor logic, key scanning, and debounce.
\end{itemize}
...
\begin{itemize}
	\item \textbf{MCAL:} low-level hardware access (GPIO, I2C, UART).
	\item \textbf{ECAL:} peripheral drivers (keypad, LCD, LED).
	\item \textbf{SRV/Application:} business logic (FSM, PIN verify, system flow).
	\item \textbf{Why useful:} easier maintenance, safer updates, better testing, and simpler scaling (new sensors or UI parts).
	\item \textbf{Example:} changing LCD address/module usually affects only the LCD driver layer.
\end{itemize}
...
\begin{itemize}
	\item Key press is scanned in the matrix and debounced.
\end{itemize}

\textbf{Phase 2: Processing ($< 1ms$)}
\begin{itemize}
	\item FSM routes key to normal mode or programming mode and updates buffers.
\end{itemize}

\textbf{Phase 3: Verification ($< 1ms$)}
\begin{itemize}
	\item On \texttt{'\#'}, input PIN is compared with stored PIN.
\end{itemize}

\textbf{Phase 4: Response (20-35ms)}
\begin{itemize}
	\item LCD shows result and LEDs indicate grant/deny, then system returns to welcome.
\end{itemize}
...
\begin{itemize}
	\item \textbf{Direct numeric mode:} map \texttt{1..9} to common menu items for fast access.
	\item \textbf{Navigation mode:} use \texttt{2/8} (up/down), \texttt{\#} (select), \texttt{*} (back).
	\item \textbf{Function keys:} reserve \texttt{A/B/C/D} for context actions (add/back/clear/home).
	\item \textbf{Usability rules:} keep key meaning consistent, show hints on LCD, and ask confirmation for critical actions.
\end{itemize}
...
\begin{itemize}
	\item \textbf{STDIO approach:} cleaner and faster to write (\texttt{printf}/\texttt{scanf}), good for readability and portability.
	\item \textbf{Direct control:} better timing and lower overhead, but more code and lower abstraction.
	\item \textbf{For security systems:} use direct control in critical real-time paths, and STDIO for logs/debug/user text flow.
	\item \textbf{Trade-off:} STDIO improves maintainability; direct control improves deterministic performance.
\end{itemize}
...
\begin{itemize}
	\item Define clear interfaces (e.g., \texttt{ILcd}, \texttt{IKeypad}, \texttt{IHal}) and code against interfaces, not board APIs.
	\item Keep board-specific code in one HAL layer (GPIO, I2C, timers, delays).
	\item Store pins/timing/address values in config files per platform.
	\item Use build flags/factory selection to bind the correct implementation at compile time.
	\item Keep application/FSM logic platform-agnostic for easy migration.
\end{itemize}
...
\begin{itemize}
	\item \textbf{Unit tests:} keypad scan/debounce, LCD output, LED control, PIN comparison.
	\item \textbf{Integration tests:} full flow from key press to LCD/LED response.
	\item \textbf{Security tests:} wrong PIN retries, brute-force lockout policy, programming-mode protection.
	\item \textbf{Robustness tests:} noisy keys, power reset during input, long run stability.
	\item \textbf{Acceptance tests:} verify user-visible behavior and response times against requirements.
\end{itemize}
